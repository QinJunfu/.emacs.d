...
- **必须使用 Emacs 27.1 或更高版本。** `init.el` 会在启动时检查版本，低于 27.1 会直接报错退出。
- 建议使用 Emacs 28.1 或更高版本。在 27.x 上可以启动，但部分功能可能不可用，启动时会给出提示。
- 首次启动需要能够访问 GNU ELPA 和 MELPA，以安装配置中的插件。
- Emacs 28.1 起已内置 `use-package`，无需额外安装；在 27.x 上它必须可用，否则需要先手动安装：
